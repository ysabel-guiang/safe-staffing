...
const Company = (props) => {
  ...
  const [teamMembers, setMembers] = useState([
    { employeeId: '', role: '', name: '', contact: '', email: '', hours: '' },
  ])
    ...
    return teamMembers.map((member) => (
      <Card key={member.employeeId}>
        <Card.Content>
          ...
          <Card.Description>Role: {member.role}</Card.Description>
          <Card.Description>Email: {member.email}</Card.Description>
          <Card.Description>Contact: {member.contact}</Card.Description>
        </Card.Content>
    ...
    return teamMembers.map((member) => (
      <>
        <Header as="h5" key={Number(member.employeeId) + 1}>
          {member.name}
          <Header.Subheader>{member.role}</Header.Subheader>
        </Header>
        <Progress
          percent={hoursToPercentage(member.hours)}
          color="blue"
          progress
          key={Number(member.employeeId) + 2}
        />
      </>
    ))
  }
  ...
  return (
    <>
      <Grid.Column width={10}>
        ...
        <Card.Group itemsPerRow={3}>{listTeamMemberCards()}</Card.Group>
